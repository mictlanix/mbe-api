...
> **Known limitation**: list endpoints for payments, quotes, refunds, credit notes and cash sessions compute their derived figures one row at a time, which is an N+1 on a full page. `/sales-orders` was converted to batch a page into two queries; the other five were not, and should follow. `SystemObject.POS` (44) governs only the cash-session routes — no point-of-sale endpoints exist, by design.
