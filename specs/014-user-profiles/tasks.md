...
- [ ] T010 **NOT DONE — needs your go-ahead.** Applying DDL writes to a shared database, and deployment is gated on confirmation. Apply `014` to a **copy** of the database and verify against [quickstart.md](./quickstart.md)'s migration block: both tables exist with the right collation, `user.profile` is `int(11)` nullable with an FK, `SELECT COUNT(*) FROM user_profile` is `0`, and `COUNT(*) FROM user WHERE profile IS NOT NULL` is `0`. Apply twice to prove idempotence, then apply the rollback and confirm the schema returns to its prior shape
...
- [ ] T051 **NOT DONE — needs a running server and T010 first.** Walk [quickstart.md](./quickstart.md) end to end against a real server: Gate 0, the migration block, all nine scenarios, and the rollback trap
