...
- [ ] T095 **PARTIAL** — the facility row lock is verified against the live database: a second caller blocked 2.14s while the first held the lock for 2.0s, proving `assign_folio` serialises concurrent callers. Still outstanding, because it requires writing rows: two confirms yielding *different* serials, and the over-refund check. Run the two concurrency checks from [quickstart.md](./quickstart.md#concurrency-checks) against a real database: two simultaneous confirms yield different serials (SC-005), and two simultaneous refunds cannot over-refund a line (SC-006)
