...
- [X] T016c Apply the migration (`uv run python -m app.db.migrate`) and confirm `status` reports it applied. **Not reversible** — the rollback drops the indexes only, it does not restore the corrected data
...
- [X] T093 [P] All six list endpoints batch their derived figures per page instead of per row: `attach_summary_totals` (orders, quotes, refunds), `attach_summary_unapplied` (payments), `attach_summary_remaining` (credit notes), `attach_summary_amounts` (cash sessions). `tests/unit/test_list_query_counts.py` asserts the query count stays flat across page sizes of 1, 5 and 50, so a reintroduced loop fails the suite. No list expands FKs, so `fk_expansion` is not needed here
...
- [X] T095 Both concurrency invariants verified against the live database. SC-005: the facility row lock serialises callers (second blocked 2.14s while the first held 2.0s), and two simultaneous folio assignments with inserts produced 56975 and 56976 — distinct, both committed, rows deleted afterwards. SC-006: after a completed refund of 2.5 on a line of 5, a second reader saw exactly 2.5 refundable (2.5 + 2.5 = 5.0), rows deleted and the figure restored from [quickstart.md](./quickstart.md#concurrency-checks) against a real database: two simultaneous confirms yield different serials (SC-005), and two simultaneous refunds cannot over-refund a line (SC-006)
- [X] T096 Scenarios 1 and 3 walked end to end against the live database at the service layer: draft -> line -> confirm (serial 47176) -> pay -> balance 0/paid -> cancel refused -> reverse without a reason refused 422 -> reverse with a reason (balance restored, 1 incidence, application still on record) -> cancel succeeds. All created rows deleted. Scenario 3's four lifecycle rules verified 4/4 against real orders read-only. **This surfaced two production bugs, both fixed — see the margin-rate and multiple-open-session entries in CHANGELOG.** Scenario 2: session opened with an opening amount, second on the same drawer refused 409, closed with denomination counts (2 rows / 1100), `current` returning none afterwards, double-close refused, and a backdated session correctly reported `stale`. Scenario 4: refund pre-populated at quantity 0 with refundable 4, excess quantity refused 422, confirm without an open session refused 409, credit-note payout issuing note 783 for 2000 with its backing payment while the source order **stayed paid**, and cash payout recording -1000 against the session with zero credit notes. Scenario 5: quote confirmed at folio 700, duplicated to a fresh draft with re-fetched prices, converted to an order carrying `origin_quote`, expired and cancelled conversions both refused 409, then credit note 784 listed at 2000, redeemed for 1000 (remaining 1000, order balance 3000 -> 2000) and reversed back to exactly 2000 / 3000. SC-003 verified on a stock-verified product: on-hand 40 -> 37 on confirm, back to 40 on cancel, the -3 entry intact with a +3 reversal appended. Every created row deleted; leftover counts checked at zero after each scenario
