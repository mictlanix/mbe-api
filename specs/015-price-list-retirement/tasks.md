...
- [~] T021 **Partially done — steps 3–5 blocked.** Steps 1 and 2 ran and pass: the suite is green (2055 passed, 15 skipped, up 28 from T001's baseline) and the split prints `DELETE product_price.list` / `BLOCK customer.price_list`, the two relations and no others. Steps 3–5 need `mbe_dev`, and the MariaDB server is down — `Can't connect to MySQL server` on the socket `.env` names — so no counts were measured against real data and no request went through a running app. What that leaves unverified is narrow but real: the MariaDB dialect's rendering of the cascade's `list` identifier, and the shape of the data at scale. `tests/integration/` covers the same behaviour against a real schema with foreign keys enforced, but on SQLite — the dialect gap `tests/integration/conftest.py` documents. Run steps 3–5 before merging if the server can be brought up.
